...
public class SoundManager {
    ...
    public void loadAudioSample(final String resourcePath, final Object sampleName) {
        try {
            ...
            logger.debug(String.format("Failed to load audio sample '%s' as '%s'", resourcePath, sampleName), e);
        } catch (final UnsupportedAudioFileException e) {
            logger.debug(String.format("Audio sample '%s' (%s) is an unsuported audio format", resourcePath, sampleName), e);
        } catch (final LineUnavailableException e) {
            logger.debug(String.format("Line unavailable for audio sample '%s' (%s)", resourcePath, sampleName), e);
        }
    }
        ...
        private void playSound(final Object soundKey) {
            ...
            if (sample != null) {
                try {
                    ...
                    logger.error(String.format("Failed to play back audio sample '%s'", soundKey), e);
                }
            } else {
