...
class CellCapturePath {
    ...
    public void execute(final OthelloPiece[][] grid) {
        ...
        logger.info(String.format("CapturePath: %d->%d, dx = %d, dy = %d",
                OthelloBoard.computeCellNumber(columnStart, rowStart), OthelloBoard.computeCellNumber(columnEnd, rowEnd), dx, dy));
        logger.info(String.format("CapturePath: setting %d with %s", OthelloBoard.computeCellNumber(x, y), piece));
        grid[x][y] = piece;
        ...
        while ((x != columnEnd) || (y != rowEnd)) {
            ...
            if ((x >= 0) && (x < OthelloBoard.COLUMNS) && (y >= 0) && (y < OthelloBoard.ROWS)) {
                logger.info(String.format("CapturePath: setting %d with %s", OthelloBoard.computeCellNumber(x, y), piece));
                grid[x][y] = piece;
            } else {
                logger.error(String.format("Move is out of bounds - %d", OthelloBoard.computeCellNumber(x, y)));
            }
        }
    }
    ...
    public String toString() {
        final int fromCell = OthelloBoard.computeCellNumber(columnStart, rowStart);
        final int toCell = OthelloBoard.computeCellNumber(columnEnd, rowEnd);
