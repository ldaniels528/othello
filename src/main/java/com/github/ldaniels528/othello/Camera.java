package com.github.ldaniels528.othello;
...
class Camera {
    ...
    public static final int X_OFFSET = (GameDisplayPane.BOARD_WIDTH - GameDisplayPane.BOARD_HEIGHT) / 2;
    public static final int Y_OFFSET = 0;
    private static final int PIECE_WIDTH = CELL_WIDTH - 6;
    private static final int PIECE_HEIGHT = CELL_HEIGHT - 6;
    private static final int GRID_WIDTH = CELL_WIDTH * OthelloBoard.COLUMNS;
    private static final int GRID_HEIGHT = CELL_HEIGHT * OthelloBoard.ROWS;
    ...
    private void renderGamePieces(final OthelloBoard board) {
        ...
        // draw the pieces
        for (int col = 0; col < OthelloBoard.COLUMNS; col++) {
            for (int row = 0; row < OthelloBoard.ROWS; row++) {
                // cache the piece
    ...
    private void renderGameBoard(final OthelloBoard board) {
        ...
        for (int col = 0; col < OthelloBoard.COLUMNS; col++) {
            for (int row = 0; row < OthelloBoard.ROWS; row++) {
                ...
                final int cellNo = OthelloBoard.computeCellNumber(col, row);
     ...
     * @param statistics the {{@link OthelloBoard.GridStatistics}}
     */
    private void renderGameInfo(final OthelloBoard.GridStatistics statistics) {
        // get the counts
    ...
    private void renderGameRecords(final OthelloBoard board) {
        ...
        offScreen.drawString(String.format("Wins: %d, Losses: %d, Draws: %d",
                board.wins, board.losses, board.draws), 160, GameDisplayPane.BOARD_HEIGHT - 20);
    }
